...
const projects = {
  fullstack: [
    ...
    {
      title: "ElectroFix",
      description: "ElectroFix serves as a comprehensive digital bridge between homeowners, professional technicians, and specialized hardware vendors. The platform streamlines the entire property maintenance lifecycle by offering on-demand service booking alongside a robust multi-vendor marketplace.",
    ...
    },
    {
      ...
      description: "This finance management ecosystem empowers users to take full control of their fiscal health through intelligent automation and real-time data visualization. By integrating advanced OCR (Optical Character Recognition) technology, the platform eliminates manual data entry.",
  ...
  backend: [
    {
      ...
      description: "This service is a high-performance, enterprise-grade recommendation engine designed to deliver deeply personalized consumer experiences through advanced semantic understanding. By leveraging Vector Databases and ANN search.",
    ...
    {
      title: "WorkSphere",
      description: "This is a project management backend engineered to streamline complex organizational workflows and enhance cross-functional team collaboration. Built as a robust ASP.NET service.",
  ...
  aiml: [
    {
      title: "VisionAI",
      description: "VisionAI is an advanced computer vision ecosystem designed to transform raw visual data into actionable intelligence through real-time object detection and spatial analysis.",
    ...
    {
      title: "FaceForge",
      description: "This is computer vision app to perform high-precision facial analysis and real-time image manipulation. By extracting granular facial landmarks creating a digital mesh.",
...
function ProjectCard({ children, accent, isSelected, onClick }) {
  ...
  return (
    <div
      ...
      className={`bg-card border border-border rounded-xl overflow-hidden transition-all duration-500 cursor-pointer ${isSelected ? 'relative z-[100] scale-105 md:scale-110 shadow-2xl' : 'z-10'}`}
      style={{
        boxShadow: (hov || isSelected) ? `0 20px 40px rgba(0,0,0,0.15), 0 0 0 1px ${accent}25` : "0 1px 3px rgba(0,0,0,0.05)",
        borderColor: (hov || isSelected) ? `${accent}60` : undefined,
        transform: isSelected ? "scale(1.05) translateY(-5px)" : hov ? "translateY(-3px)" : "none",
      }}
    >
      {children}
    </div>
...
function CardFooter({ children }) {
  return (
    <div className="flex items-center gap-2 pt-2.5 border-t border-border mt-0.5" onClick={(e) => e.stopPropagation()}>
      {children}
    </div>
  );
}
...
function FullStackCard({ p, isSelected, onClick }) {
  return (
    <ProjectCard accent="#7c3aed" isSelected={isSelected} onClick={onClick}>
...
function BackendCard({ p, isSelected, onClick }) {
  return (
    <ProjectCard accent="#2563eb" isSelected={isSelected} onClick={onClick}>
      <CardBody>
...
function AiMlCard({ p, isSelected, onClick }) {
  const isShowImage = p.showImage;
  return (
    <ProjectCard accent="#a21caf" isSelected={isSelected} onClick={onClick}>
      {isShowImage && (
...
export default function ProjectsTab() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans py-6 md:py-10 px-4 md:px-6 pb-20 relative">
      <style>{`
        @keyframes fadeUp { from { opacity:0; transform:translateY(14px); } to { opacity:1; transform:translateY(0); } }
        .card-enter { animation: fadeUp 0.3s ease both; }
        .hide-scrollbar::-webkit-scrollbar { display: none; }
        .hide-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
        .blur-content { filter: blur(5px); pointer-events: none; transition: filter 0.4s ease; }
      `}</style>

      {/* Backdrop for Blur Effect */}
      {selectedProject && (
        <div 
          className="fixed inset-0 z-[90] bg-background/20 backdrop-blur-[2px] transition-all duration-500"
          onClick={() => setSelectedProject(null)}
        />
      )}

      <div className={`max-w-[1060px] mx-auto transition-all duration-500 ${selectedProject ? 'blur-content' : ''}`}>
        <div className="mb-7">
            ...
            {tabs.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => switchTab(tab.id)}
                  ...
                  <span className="text-muted-foreground">{tab.emoji}</span>
                  <span className={isActive ? "" : "text-muted-foreground"}>{tab.label}</span>
                  <span className={`text-[9px] md:text-[10px] font-bold rounded-full px-1.5 py-0.5 transition-all duration-150 ${isActive ? "" : "bg-muted text-muted-foreground"}`}
                    style={{
      ...
      {/* Grid - Render outside the blur container to allow the selected one to stay sharp */}
      <div className="max-w-[1060px] mx-auto relative z-[95]">
        <div key={animKey}>
          <div className="grid gap-4 md:gap-6" 
               style={{ gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 300px), 1fr))" }}>
            {activeTab === "fullstack" && projects.fullstack.map((p, i) => (
              <div key={p.title} 
                className={`card-enter ${selectedProject && selectedProject !== p.title ? 'blur-content' : ''}`} 
                style={{ animationDelay: `${i * 0.07}s` }}>
                <FullStackCard 
                  p={p} 
                  isSelected={selectedProject === p.title} 
                  onClick={() => setSelectedProject(selectedProject === p.title ? null : p.title)}
                />
              </div>
            ))}
